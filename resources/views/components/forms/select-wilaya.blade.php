...
<select name="{{ $name }}"
    {{ $attributes->merge(['class' => 'bg-gray-100 text-gray-700 border border-gray-200 focus:outline-none focus:bg-white focus:border-gray-500 py-2 rounded-md', 'required']) }}>

    <option disabled selected value> -- Séléctionnez une wilaya -- </option>

    @foreach ($wilayas as $wilaya)
        <option value="{{ $wilaya['name'] }}" @selected($selectedWilaya === $wilaya['name'])> {{ $wilaya['id'] }}-
            {{ $wilaya['name'] }} </option>
    @endforeach
